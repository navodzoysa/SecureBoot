...
const char *server = "secureboot.online";
const char *url = "https://secureboot.online/api/firmware/download/6457a5ed32689715b47f923b";
...
void setup() {
  ...
  Serial.printf("Booted %s with ChipID - %" PRIu64 "\n", ESP.getChipModel(),
                ESP.getEfuseMac());
  Serial.printf("Firmware Version - %s", firmwareVersion);
  ...
  String latestVersion = httpsHandlder.checkNewFirmware(firmwareVersion);
  Serial.printf("New firmware - %s\n", latestVersion);
  // httpsHandlder.downloadFirmware(url);
}
