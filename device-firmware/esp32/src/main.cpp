...
void setup() {
  ...
  if (firmwareVersion == latestVersion.c_str() || latestVersion == "latest") {
    Serial.println("Firmware is up to date.");
    return;
  }

  if (latestVersion == "error" || latestVersion == "") {
    Serial.println("Error fecthing latest firmware version.");
    return;
  }

  httpsHandlder.downloadFirmware(latestVersion.c_str(),
